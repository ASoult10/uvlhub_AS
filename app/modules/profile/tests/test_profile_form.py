...
def test_orcid_is_optional(app):
    with app.app_context():
        form = UserProfileForm(data={"name": "John", "surname": "Doe", "orcid": "", "affiliation": "Uni"})
...
def test_orcid_invalid_format(app):
    with app.app_context():
        form = UserProfileForm(
            formdata=MultiDict(
                {"name": "John", "surname": "Doe", "orcid": "1234-5678-999", "affiliation": "Uni"}  # too short
            )
        )
...
def test_orcid_valid(app):
    with app.app_context():
        form = UserProfileForm(
            data={"name": "Alice", "surname": "Smith", "orcid": "0000-1111-2222-3333", "affiliation": "Uni"}
        )
        assert form.validate() is True
...
def test_required_fields_missing(app):
    with app.app_context():
        form = UserProfileForm(data={"name": "", "surname": "", "orcid": "", "affiliation": "Uni"})
