...
def manage_keys():
    ...
    return render_template("api/manage_keys.html", form=form, revoke_form=revoke_form, api_keys=user_keys)
...
def get_dataset_by_id(api_key_obj, id):
    ...
    try:
        ...
        if not dataset:
            return jsonify({"error": "not_found", "message": f"Dataset with id {id} not found"}), 404
            ...
            jsonify(
                {
                    ...
                    "publication_type": (
                        metadata.publication_type.value if metadata and metadata.publication_type else None
                    ),
...
def get_dataset_by_title(api_key_obj, title):
    ...
    try:
        ...
        metadata = DSMetaData.query.filter(DSMetaData.title.ilike(f"%{search_title}%")).first()

        if not metadata:
            return (
                jsonify({"error": "not_found", "message": f'No dataset found with title containing "{search_title}"'}),
                404,
            )
        ...
        if not dataset:
            return (
                jsonify({"error": "not_found", "message": "Dataset metadata found but dataset record is missing"}),
                404,
            )
...
def list_datasets(api_key_obj):
    ...
    try:
        ...
        return jsonify({"total": len(results), "datasets": results}), 200
...
def search_datasets(api_key_obj):
    ...
    try:
        ...
        metadata_results = DSMetaData.query.filter(
            DSMetaData.title.ilike(f"%{query}%") | DSMetaData.description.ilike(f"%{query}%")
        ).all()
        ...
        if not metadata_ids:
            return (
                jsonify(
                    {"query": query, "total": 0, "page": page, "per_page": per_page, "total_pages": 0, "results": []}
                ),
                200,
            )
...
def api_stats(api_key_obj):
    ...
    return jsonify({"total_datasets": DataSet.query.count(), "version": "1.0"}), 200
